...
$(document).ready(() => {
    ...
    app.titleChanger(['y', 'yo', 'yov', 'yovr', 'yovra', 'yovrah', 'yovrah.github.io', 'prxz - love is more depressing than depression']);
...
$.getJSON(ipgeolocation, (data) => {
    writeLine(['<i style="color: #0f0">Authentication...</i>', "Предоставление доступа к <i><span style='font-size: 14px; color: #ff0000;'>[unknown]</span></i>..."], 30, 500, () => {
        ...
        writeLine([`<b style='color: #0f0'>login: yovrah</b>`, `<b style='color: #0f0'>password: abfz36fN3bv5nP2a;</b>`, `Доступ разрешен! <span style='font-size: 14px; color: #0f0;'><i>[success]</i></span>`, `Добро пожаловать обратно, <i style='color: #00fff5'>${ip}</i>! Кстати, приятно видеть кого-то из ${country}`], 30, 300, () => {
            if (app.skippedIntro) return;

            clearCursor();

            writeLine([`<i style='color: #ff00d8'; font-size: 20px;>/id/yovrah</i>`], 50, 200, () => {
                timeouts.push(
                    setTimeout(() => {
                        if (app.skippedIntro) return;
